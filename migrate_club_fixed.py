...
duplicate_locations = club_df['location'].value_counts()
duplicate_locations = duplicate_locations[duplicate_locations > 1].index.tolist()

for location in duplicate_locations:
    # العثور على السجلات التي تحتوي على نفس القيمة في عمود location
    duplicate_indices = club_df[club_df['location'] == location].index[1:]
    
    # تعديل قيم location للسجلات المكررة
    for idx in duplicate_indices:
        club_df.at[idx, 'location'] = f"{location} {uuid.uuid4().hex[:8]}"

# حفظ البيانات في ملف CSV للاحتفاظ بها
club_df.to_csv('club_backup_fixed.csv', index=False, encoding='utf-8-sig')
print("تم حفظ بيانات جدول club في ملف club_backup_fixed.csv")
